...
In order to be able to understand the underlying mechanisms causing the observed phenomena we need to define a methodology to observe under-the-hoods of the training procedure of neural networks.
...
We used the MNIST and CIFAR-10 datasets throughout the experiments. These datasets are among the most widely used benchmarks for image classification and provide two complementary levels of visual complexity. The MNIST dataset consists of 70,000 grayscale images of handwritten digits (60,000 training and 10,000 testing images), where each image has a resolution of $28 \times 28$ pixels and belongs to one of ten digit classes (0–9).

The CIFAR-10 dataset contains 60,000 colour images (50,000 training and 10,000 testing images), each with a resolution of $32 \times 32$ pixels and belonging to one of ten object categories, including airplanes, automobiles, birds, cats, deer, dogs, frogs, horses, ships, and trucks. Compared to MNIST, CIFAR-10 presents a substantially more challenging classification task because of its greater visual diversity, colour information, and more complex object features. Using both datasets enables the evaluation of neural network plasticity under learning tasks of varying complexity while maintaining a consistent ten-class classification setting.
\begin{table}[h]
\centering
...
As mentioned above, the primary objective of this work is not to maximize reinforcement learning performance or model accuracy, but rather to understand and quantify the phenomenon of plasticity loss. Therefore, a precise operational definition of plasticity is required.

Following \citep{lyle2023understanding}, plasticity is defined as the ability of a neural network to update its predictions in response to new learning signals. More formally, consider an optimization algorithm:
\begin{equation}
...
To measure the flexibility of a network, a distribution over learning objectives is considered. In particular, \citep{lyle2023understanding} define a family of regression objectives
...
The following definition is used to quantify this adaptability
\citep{lyle2023understanding}. Let $\mathcal{L}$ denote a distribution
over probe loss functions. For a network checkpoint $\theta_t$ and a
sampled probe objective $\ell \sim \mathcal{L}$, the optimization
procedure produces the adapted parameters

\begin{equation}
\theta_t^{*}
=
\mathcal{O}(\theta_t,\ell),
\end{equation}

where $\mathcal{O}$ is the same optimization procedure used for the
network's primary learning objective. The adapted parameters
$\theta_t^{*}$ need not be a global optimum; they are simply the
parameters obtained after a fixed optimization budget.

Let $b$ denote the loss achieved by a baseline predictor. For a
regression objective, $b$ may, for example, be chosen as the variance
of the target values. The plasticity of the network at checkpoint
$\theta_t$ is then defined as
...
b
-
\mathbb{E}_{\ell \sim \mathcal{L}}
\left[
    \ell\left(\theta_t^{*}\right)
\right],
\qquad
\theta_t^{*}
=
...
A network is considered more plastic when it can obtain a lower loss on the sampled probe objectives within the fixed optimization budget. Consequently, a larger value of $\mathcal{P}(\theta_t)$ indicates greater plasticity.

Following \citet{lyle2023understanding}, the change in plasticity
relative to initialization is defined as
...
-
\mathcal{P}(\theta_0),
\label{eq:plasticity-change}
\end{equation}

where $\theta_0$ denotes the randomly initialized network. Because the baseline $b$ appears in both terms, it cancels:
...
\mathbb{E}_{\ell \sim \mathcal{L}}
\left[
    \ell\left(\theta_0^{*}\right)
    -
    \ell\left(\theta_t^{*}\right)
\right].
\end{equation}

Under this signed convention, $\Delta\mathcal{P}_t = 0$ indicates that
plasticity has not changed relative to initialization,
$\Delta\mathcal{P}_t < 0$ indicates plasticity loss, and
$\Delta\mathcal{P}_t > 0$ indicates improved adaptability.
More negative values therefore correspond to more severe plasticity
loss.
...
\begin{figure}[t]
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \caption{Illustration of a loss landscape together with the principal eigenvectors of the Hessian matrix. The eigenvectors indicate the directions of maximum and minimum curvature, while the corresponding eigenvalues quantify the curvature along those directions. Image generated using the generative model GPT5.6.}
